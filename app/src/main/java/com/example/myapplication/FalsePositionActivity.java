...
public class FalsePositionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_falsi);
    ...
    private void calculateFalsePosition() {
        ...
        String result = null;
        TableLayout iterationTable = null;
        if (f(xl, equation) * f(xr, equation) > 0.0) {
            ...
            while (condition) {
                x2 = x0 - (f(x0, equation) * (x1 - x0)) / (f(x1, equation) - f(x0, equation));
            ...
            double root = x2;
            result = String.format("Root Value is: %.4f", root);
        }
        ...
        if (iterationTable != null) {
            ...
            // Handle the case when iterationTable is null
